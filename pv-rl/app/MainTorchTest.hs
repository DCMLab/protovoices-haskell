...
import Torch.Vision qualified as V
...
trainLoop model optimizer = P.foldM step begin done . enumerateData
 where
  ...
  step model ((input, label), iter) = do
    let loss = nllLoss' (toDevice dev label) $ mlp model (toDevice dev input)
...
displayImages model (testImg, testLabel) = do
  V.dispImage testImg
  putStrLn $ "Model        : " ++ (show . (argmax (Dim 1) RemoveDim) . exp $ mlp model $ toDevice dev testImg)
