...
{- | Checks if 't' contains any NaNs. If yes, then throws an error that shows 't' and 'ctx'.
'ctx' can contain any contextual information that is considered useful,
e.g., a string that indicates where the check is performed.
-}
checkNaN ctx t =
